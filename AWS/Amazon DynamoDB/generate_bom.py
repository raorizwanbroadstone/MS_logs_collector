...
def extract_resource_inventory(event: dict) -> dict | None:
    if event.get("EventSource") != "dynamodb-local-enumeration":
        return None
    ...
    return {
        ...
        "arn":               inv.get("arn", ""),
        "table_id":          inv.get("table_id", ""),
        "table_status":      inv.get("table_status", ""),
        "billing_mode":      inv.get("billing_mode", ""),
        "item_count":        inv.get("item_count", 0),
        "size_bytes":        inv.get("size_bytes", 0),
        "partition_key":     inv.get("partition_key", ""),
        "sort_key":          inv.get("sort_key", ""),
        "gsi_count":         inv.get("gsi_count", 0),
        "lsi_count":         inv.get("lsi_count", 0),
        "stream_enabled":    inv.get("stream_enabled", False),
        "stream_view_type":  inv.get("stream_view_type", ""),
        "encryption_type":   inv.get("encryption_type", ""),
        "creation_datetime": inv.get("creation_datetime", ""),
        "pitr_enabled":      inv.get("pitr_enabled", False),
        "pitr_earliest":     inv.get("pitr_earliest", ""),
        "pitr_latest":       inv.get("pitr_latest", ""),
        "access_denied":     inv.get("access_denied", False),
        "not_found":         inv.get("not_found", False),
    }


def extract_dynamodb_resource(event: dict) -> dict | None:
    if event.get("EventSource") == "dynamodb-local-enumeration":
        return None
    params = event.get("requestParameters") or {}
    ...
    if not resource_name:
        for res in (event.get("Resources") or []):
            if isinstance(res, dict) and "DynamoDB" in res.get("type", ""):
                arn           = res.get("ARN", "")
...
def to_cyclonedx_service(raw: dict, inventory: dict | None = None) -> dict:
    svc: dict = {
        "bom-ref":       _make_bom_ref("dynamodb_resource", raw["key"]),
        "name":          raw["name"],
        "authenticated": True,
    }
    props = [
        {"name": "aws:DynamoDBResourceType", "value": raw.get("resource_type", "")},
        {"name": "aws:DynamoDBResourceName", "value": raw.get("resource_name", "")},
    ...
    if inventory:
        if inventory.get("access_denied"):
            ...
            if inventory.get("arn"):
                props.append({"name": "aws:DynamoDBTableArn",   "value": inventory["arn"]})
            if inventory.get("table_id"):
                props.append({"name": "aws:TableId",            "value": inventory["table_id"]})
            if inventory.get("table_status"):
                props.append({"name": "aws:TableStatus",        "value": inventory["table_status"]})
            if inventory.get("billing_mode"):
                props.append({"name": "aws:BillingMode",        "value": inventory["billing_mode"]})
            props.append({"name": "aws:ItemCount",               "value": str(inventory.get("item_count", 0))})
            size_mb = round(inventory.get("size_bytes", 0) / (1024 * 1024), 2)
            props.append({"name": "aws:TableSizeMB",             "value": str(size_mb)})
            if inventory.get("partition_key"):
                props.append({"name": "aws:PartitionKey",       "value": inventory["partition_key"]})
            if inventory.get("sort_key"):
                props.append({"name": "aws:SortKey",            "value": inventory["sort_key"]})
            if inventory.get("gsi_count"):
                props.append({"name": "aws:GSICount",           "value": str(inventory["gsi_count"])})
            if inventory.get("lsi_count"):
                props.append({"name": "aws:LSICount",           "value": str(inventory["lsi_count"])})
            props.append({"name": "aws:StreamEnabled",           "value": str(inventory.get("stream_enabled", False)).lower()})
            if inventory.get("stream_view_type"):
                props.append({"name": "aws:StreamViewType",     "value": inventory["stream_view_type"]})
            if inventory.get("encryption_type"):
                props.append({"name": "aws:EncryptionType",     "value": inventory["encryption_type"]})
            if inventory.get("creation_datetime"):
                props.append({"name": "aws:CreationDateTime",   "value": inventory["creation_datetime"]})
            props.append({"name": "aws:PITREnabled",             "value": str(inventory.get("pitr_enabled", False)).lower()})
            if inventory.get("pitr_earliest"):
                props.append({"name": "aws:PITREarliestRestore","value": inventory["pitr_earliest"]})
            if inventory.get("pitr_latest"):
                props.append({"name": "aws:PITRLatestRestore",  "value": inventory["pitr_latest"]})
...
def build_dependency_graph(
    ...
    resource_ref_map = {r["key"]: _make_bom_ref("dynamodb_resource", r["key"]) for r in raw_services}
        ...
        "metadata": {
            ...
            "tools": {
                "components": [
                    {
                        "type":    "application",
                        "name":    "aws-dynamodb-bom-generator",
                        "version": "1.0.0",
                    }
                ]
            },
...
def process_log_file(
    ...
    for event in stream_events(log_file):
        ...
        resource = extract_dynamodb_resource(event)
