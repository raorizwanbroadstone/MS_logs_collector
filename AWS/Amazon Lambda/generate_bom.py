...
def extract_resource_inventory(event: dict) -> dict | None:
    if event.get("EventSource") != "lambda-local-enumeration":
        return None
    ...
    return {
        ...
        "memory_size":         inv.get("memory_size", 0),
        "timeout":             inv.get("timeout", 0),
        "last_modified":       inv.get("last_modified", ""),
        "state":               inv.get("state", ""),
        "architecture":        inv.get("architecture", ""),
        "package_type":        inv.get("package_type", ""),
        "layer_arns":          inv.get("layer_arns", []),
        "vpc_enabled":         inv.get("vpc_enabled", False),
        "log_group":           inv.get("log_group", ""),
        "compatible_runtimes": inv.get("compatible_runtimes", []),
        "access_denied":       inv.get("access_denied", False),
        "not_found":           inv.get("not_found", False),
    }


def extract_lambda_resource(event: dict) -> dict | None:
    if event.get("EventSource") == "lambda-local-enumeration":
        return None
    params = event.get("requestParameters") or {}
    ...
    if not resource_name:
        for res in (event.get("Resources") or []):
            if isinstance(res, dict) and "Lambda" in res.get("type", ""):
                arn           = res.get("ARN", "")
...
def to_cyclonedx_service(raw: dict, inventory: dict | None = None) -> dict:
    svc: dict = {
        "bom-ref":       _make_bom_ref("lambda_resource", raw["key"]),
        "name":          raw["name"],
        "authenticated": True,
    }
    props = [
        {"name": "aws:LambdaResourceType", "value": raw.get("resource_type", "")},
        {"name": "aws:LambdaResourceName", "value": raw.get("resource_name", "")},
    ...
    if inventory:
        if inventory.get("access_denied"):
            ...
            if inventory.get("arn"):
                props.append({"name": "aws:LambdaResourceArn", "value": inventory["arn"]})
            if rtype == "Function":
                if inventory.get("runtime"):
                    props.append({"name": "aws:Runtime",      "value": inventory["runtime"]})
                if inventory.get("handler"):
                    props.append({"name": "aws:Handler",      "value": inventory["handler"]})
                if inventory.get("memory_size"):
                    props.append({"name": "aws:MemorySize",   "value": f"{inventory['memory_size']} MB"})
                if inventory.get("timeout"):
                    props.append({"name": "aws:Timeout",      "value": f"{inventory['timeout']}s"})
                if inventory.get("architecture"):
                    props.append({"name": "aws:Architecture", "value": inventory["architecture"]})
                if inventory.get("package_type"):
                    props.append({"name": "aws:PackageType",  "value": inventory["package_type"]})
                if inventory.get("state"):
                    props.append({"name": "aws:State",        "value": inventory["state"]})
                if inventory.get("last_modified"):
                    props.append({"name": "aws:LastModified", "value": inventory["last_modified"]})
                if inventory.get("vpc_enabled"):
                    props.append({"name": "aws:VpcEnabled",   "value": "true"})
                if inventory.get("log_group"):
                    props.append({"name": "aws:LogGroup",     "value": inventory["log_group"]})
                for i, layer_arn in enumerate(inventory.get("layer_arns", [])):
                    if layer_arn:
                        props.append({"name": f"aws:Layer:{i}", "value": layer_arn})
            elif rtype == "Layer":
                if inventory.get("last_modified"):
                    props.append({"name": "aws:CreatedDate", "value": inventory["last_modified"]})
                for rt in inventory.get("compatible_runtimes", []):
                    props.append({"name": "aws:CompatibleRuntime", "value": rt})
...
def build_dependency_graph(
    ...
    resource_ref_map = {r["key"]: _make_bom_ref("lambda_resource", r["key"]) for r in raw_services}
...
def process_log_file(
    ...
    for event in stream_events(log_file):
        ...
        resource = extract_lambda_resource(event)
